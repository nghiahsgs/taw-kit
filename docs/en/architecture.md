...
Technical overview for developer-buyers who want to fork, extend, or audit the kit before deploying it internally. If you are a non-developer, start with the Vietnamese [quickstart](../vi/quickstart.md) instead.

## Overview

taw-kit is a Claude Code extension that turns Vietnamese product descriptions into shipped web applications. It ships as a single installable bundle composed of:

- **25 skills** — scoped capability modules loaded on demand by Claude Code. The root orchestrator is `skills/taw/`; the rest cover stack concerns (Next.js scaffolding, Tailwind tokens, Supabase bootstrap, payment integration, deploy handoff, error translation, etc.).
...
- **`tawkit` CLI** — a small shell wrapper for install, update, doctor, and offline maintenance tasks.
...
All user-visible output is Vietnamese by design. All internal reasoning, logs, and state files use English to keep diffs readable for auditors.
...
- **`post-tool`** — listens for tool results, updates `.taw/checkpoint.json`, and triggers auto-retry logic for transient errors.
...
**Custom presets.** Drop a new YAML into `presets/` with the shape `{category, clarify_questions, stack_defaults, what_you_get_vi}`. The orchestrator auto-discovers it and the non-dev flow gets a new category with zero code changes.

**Stack overrides.** Every stack choice in `skills/taw/SKILL.md` ("Next.js 14 + Tailwind + Supabase + Polar + Shipkit") is a default, not a mandate. If the user says "dùng Remix" or "không dùng Supabase" in Step 2, the orchestrator routes to the appropriate alternative skill. Add a new skill under `skills/<name>/` and reference it from a preset to make it discoverable.
...
taw-kit runs entirely client-side. There is no taw-kit server — nothing your Claude session produces leaves your machine except the API calls to Anthropic (for LLM) and to whatever deploy target (Vercel, Supabase, Polar, Shipkit) you explicitly authorize.
...
**Reviewer P0 checks.** The `reviewer` agent runs fixed P0 (blocker) checks before handoff to deploy: (1) no plaintext secrets in committed files, (2) no hardcoded API keys in source, (3) `.env.local` is gitignored, (4) no SQL strings that concatenate user input, (5) no `dangerouslySetInnerHTML` of uncontrolled sources. A P0 failure halts the chain and surfaces a Vietnamese error to the user — there is no bypass flag.

**Secret scanning.** The install script places a pre-commit hook (`pre-commit-scan.sh`) that greps staged diffs for token prefixes. Commit is rejected on match. This is belt-and-suspenders on top of the reviewer's check.
...
- Vietnamese user docs: [`docs/vi/quickstart.md`](../vi/quickstart.md) and [`docs/vi/troubleshooting.md`](../vi/troubleshooting.md)
